...
impl<'a> ParseLogGroup<'a> for LogGroup<'a> {
    ...
    fn new(
        ...
        LogGroup { logs, topic, source, log_tags }
    }
}
...
impl<'a> ParseLogGroup<'a> for LogGroupRaw<'a> {
    ...
    fn new(
        ...
        LogGroupRaw { logs, topic, source, log_tags }
    }
}
    ...
    L: ParseLog<'static>,
{
    ...
    let time_obj = dict.get_item("Time")
...
{
    let log_items_obj = log_group_dict.get_item("LogItems")
    ...
    let log_items_list = log_items_obj.downcast::<PyList>()
    ...
    let log_tags_obj = log_group_dict.get_item("LogTags")
    ...
    let log_tags_list = log_tags_obj.downcast::<PyList>()
    ...
    let topic = if let Ok(Some(topic_obj)) = log_group_dict.get_item("Topic") {
        extract_optional_string(&topic_obj, "Topic")?.map(|s| Cow::Owned(s))
    } else {
        None
    };
    
    let source = if let Ok(Some(source_obj)) = log_group_dict.get_item("Source") {
        extract_optional_string(&source_obj, "Source")?.map(|s| Cow::Owned(s))
    } else {
        None
    };
    ...
    log_group.write_message(&mut writer)
...
#[pyfunction]
fn serialize_log_group(py: Python<'_>, log_group_dict: &Bound<'_, PyDict>) -> PyResult<Py<PyBytes>> {
...
///     ValueError: If required fields are missing.
#[pyfunction]
fn serialize_log_group_raw(py: Python<'_>, log_group_dict: &Bound<'_, PyDict>) -> PyResult<Py<PyBytes>> {
